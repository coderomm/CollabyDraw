import {
  ...
  export class CanvasEngine {
    ...
    // Add this method to handle real-time shape streaming
    private streamShape(shape: Shape) {
      // Don't stream if we're disconnected or in standalone mode
      if (!this.isConnected || this.isStandalone) return;
  
      // Store the ID of the shape we're currently streaming
      if (!this.streamingShapeId) {
        this.streamingShapeId = shape.id;
      }
  
      // Throttle streaming updates to avoid overwhelming the WebSocket
    ...
    clearCanvas() {
      ...
      this.remoteCursors.forEach(({ x, y, userName }) => {
        const screenX = x * this.scale + this.panX;
        const screenY = y * this.scale + this.panY;
  
        // Draw a simple dot or icon
        this.ctx.beginPath();
        this.ctx.fillStyle = "#ff4081"; // pink dot
        ...
        this.ctx.fillText(userName, screenX + 8, screenY - 8);
      });
    }
        ...
        if (this.cursorThrottleTimeout === null) {
          this.cursorThrottleTimeout = window.setTimeout(() => {
            ...
            const message = {
              ...
              userName: this.userName!,
              connectionId: this.connectionId,
              message: { x: coords.x, y: coords.y },
            };
  
            try {
              if (this.socket?.readyState === WebSocket.OPEN) {
                this.socket.send(JSON.stringify(message));
                console.log("Sending streaming CURSOR_MOVE: ", message);
              }
            } catch (e) {
